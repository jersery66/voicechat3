...
class ReportService:
    ...
    def _run_with_fallback(self, prompt: str, *, label: str) -> str:
        """Run a non-streaming agent call with 72B fallback.

        Used by all the simple ``generate_report``-style entry points to
        eliminate the duplicated try/except boilerplate.
        """
        try:
            agent = self._get_agent_service()
            text = agent.generate_report(prompt, timeout=AGENT_REPORT_TIMEOUT)
            logger.debug(f"{label} generated by 3B model")
            return text
        except Exception as agent_err:
            logger.warning(f"3B {label} failed, falling back to 72B: {agent_err}")
            return self._fallback_chat(prompt, stream=False)
    ...
    def record_relaxation(self, relaxation_name: str):
        ...
        logger.info(f"ReportService recorded relaxation: {relaxation_name}")

    def record_game_session(self, game_results: dict):
        """Record therapeutic game session results."""
        self.game_results = game_results
        logger.info(f"ReportService recorded game session: {game_results}")
    ...
    def generate_researcher_report(self,
        ...
        try:
            ...
            try:
                ...
                logger.debug("Report generated by 3B model")
            except Exception as agent_err:
                logger.warning(f"3B report failed, falling back to 72B: {agent_err}")
                analysis_text = self._fallback_chat(prompt, stream=False)
        ...
        try:
            ...
            try:
                ...
                if stream:
                    ...
                    logger.debug("Visitor feedback generated by 3B model")
        ...
        try:
            ...
            try:
                ...
                if stream:
                    ...
                    logger.debug("Session summary generated by 3B model")
    ...
    def generate_session_summary_async(self, conversation_history: List[Dict], callback=None) -> str:
        ...
        try:
            ...
            try:
                agent = self._get_agent_service()
                summary = agent.generate_report(prompt, timeout=AGENT_REPORT_TIMEOUT)
                logger.debug("Session summary (async) generated by 3B model")
            except Exception as agent_err:
                logger.warning(f"3B summary_async failed, falling back to 72B: {agent_err}")
                summary = self._fallback_chat(prompt, stream=False)
            ...
            logger.error(f"generate_session_summary_async failed: {e}")
    ...
    def generate_suggestions(self, conversation_history: List[Dict]) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.debug("Suggestions generated by 3B model")
            except Exception as agent_err:
                logger.warning(f"3B suggestions failed, falling back to 72B: {agent_err}")
                suggestions = self._fallback_chat(prompt, stream=False)
...
不要解释，只返回关键词。如果不确定，默认返回 呼吸。"""
        ...
        try:
            ...
            try:
                ...
                logger.debug("Relaxation recommendation by 3B model")
            except Exception as agent_err:
                logger.warning(f"3B relaxation rec failed, falling back to 72B: {agent_err}")
                content = self._fallback_chat(prompt, stream=False).strip()
            ...
            logger.error(f"Relaxation recommendation failed: {e}")
            return "呼吸" # Fallback
